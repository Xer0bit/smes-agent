...
// ── Global Process Safety Guards for Resilient HMR ─────────────────────────
// Prevent process crashes caused by unhandled compilation/runtime rejections
process.on('uncaughtException', (err) => {
    const errorMsg = err?.message || String(err);
    console.error(`[${projectId}] Uncaught exception caught by child runner:`, errorMsg);
    onDiagnostic(`Uncaught Exception: ${errorMsg}`, 'runtime');
});

process.on('unhandledRejection', (reason) => {
    const errorMsg = reason?.message || String(reason);
    console.error(`[${projectId}] Unhandled rejection caught by child runner:`, errorMsg);
    onDiagnostic(`Unhandled Rejection: ${errorMsg}`, 'runtime');
});
