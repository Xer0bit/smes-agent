...
const STATUS_COLORS: Record<string, string> = {
  posted: 'var(--success)', scheduled: '#0ea5c9', posting: '#0ea5c9',
  draft: 'var(--muted)', failed: 'var(--danger)', cancelled: '#64748b',
};
...
function Trend({ now, prev }: { now: number; prev: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-0.5 font-medium" style={{ fontSize: 'var(--text-tiny)', color: up ? 'var(--success)' : 'var(--danger)' }}>
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-end justify-between pb-1">
        <div>
          <p className="font-bold uppercase" style={{ fontSize: 'var(--text-tiny)', letterSpacing: 'var(--tracking-wide)', color: 'var(--accent)' }}>{greeting()}</p>
          <h1 style={{ color: 'var(--text)', marginTop: '0.125rem' }}>{ECG.appName}</h1>
        </div>
          ...
          {stats!.platforms.map(p => {
            ...
            return (
              <div key={p.platform} className="flex items-center gap-2 rounded-full border px-3.5 py-2 shrink-0"
                ...
                <span className="font-medium" style={{ fontSize: 'var(--text-small)', color: 'var(--text)' }}>{pm.label}</span>
                <span className="tabular-nums" style={{ fontSize: 'var(--text-small)', color: 'var(--muted)' }}>{p.posted30d}</span>
      ...
      {setupSteps.length > 0 && !setupDone && (
        <Card className="overflow-hidden">
          <div className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--border)' }}>
            <p className="font-medium" style={{ fontSize: 'var(--text-body)', color: 'var(--text)' }}>Get started</p>
          ...
          <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
            {setupSteps.map((step, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-3" style={step.done ? { opacity: 0.4 } : undefined}>
                ...
                <p className="flex-1" style={{ fontSize: 'var(--text-body)', color: 'var(--text)' }}>{step.title}</p>
                <button onClick={() => navigate(step.path)}
                  className="flex items-center gap-0.5 transition-opacity hover:opacity-70 shrink-0" style={{ fontSize: 'var(--text-small)', color: 'var(--accent)' }}>
      ...
      {has('posts') && pending.length > 0 && (
        <button onClick={() => navigate('/posts')}
          ...
          <span className="flex-1" style={{ fontSize: 'var(--text-body)', color: 'var(--text)' }}>
      ...
      {/* ── KPI cards — the dominant row: reads first via StatCard's h1-scale
           value + accent bar, ahead of the subordinate chart/rail below. ── */}
          ...
          <StatCard label="Run success · 30d" value={successRate === null ? '—' : `${successRate}%`} Icon={Activity}
            ...
            sub={runTotal > 0 && (
              <div className="h-1 rounded-full overflow-hidden" style={{ background: 'var(--border)' }}>
                <div className="h-full rounded-full" style={{ width: `${successRate}%`, background: successRate! >= 90 ? 'var(--success)' : successRate! >= 60 ? 'var(--warning)' : 'var(--danger)' }} />
              </div>
            )} />
      ...
      {/* ── Chart + right rail — subordinate to the KPI row above: extra top
           margin sets it apart as a second tier, and SectionHeader's smaller
           label weight (vs. StatCard's h1-scale numbers) keeps it quiet. ── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{ marginTop: 'var(--space-4)' }}>
        {/* Publishing activity, weekly, top 2 platforms */}
        <Card className="p-5 lg:col-span-2">
          <SectionHeader title="Publishing activity" action={<span style={{ fontSize: 'var(--text-tiny)', color: 'var(--muted)' }}>last 90 days · weekly</span>} />
          {!chart ? (
            <p className="text-center py-10" style={{ fontSize: 'var(--text-small)', color: 'var(--muted)' }}>
            ...
            <>
              ...
              <div className="flex items-center gap-4 mt-2">
                {chart.series.map(s => {
                  const pm = platformMeta(s.platform);
                  return (
                    <span key={s.platform} className="inline-flex items-center gap-1.5" style={{ fontSize: 'var(--text-tiny)', color: 'var(--muted)' }}>
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            {upcoming.length === 0 ? (
              <p className="py-2" style={{ fontSize: 'var(--text-small)', color: 'var(--muted)' }}>Nothing scheduled. Approved posts with a future time show up here.</p>
            ) : (
                ...
                {upcoming.map((p: any) => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center gap-2.5 rounded-lg border px-3 py-2" style={{ borderColor: 'var(--border)' }}>
                      ...
                      <p className="flex-1 min-w-0 truncate" style={{ fontSize: 'var(--text-small)', color: 'var(--text)' }}>{p.content ?? '(no content)'}</p>
                      <span className="shrink-0 tabular-nums" style={{ fontSize: 'var(--text-tiny)', color: 'var(--muted)' }} title={new Date(at).toLocaleString()}>
                        {relTime(at)}
                      </span>
                    </div>
                  );
          ...
          <Card className="p-5">
            ...
            {!pipeline ? (
              <p className="py-2" style={{ fontSize: 'var(--text-small)', color: 'var(--muted)' }}>No posts yet.</p>
            ) : (
              <div className="flex items-center gap-4">
                <div className="relative w-20 h-20 shrink-0 rounded-full" style={{
                  ...
                  <div className="absolute inset-[22%] rounded-full flex items-center justify-center" style={{ background: 'var(--card-bg)' }}>
                    <span className="font-bold" style={{ fontSize: 'var(--text-small)', color: 'var(--text)' }}>{pipeline.total}</span>
                  </div>
                </div>
                <div className="space-y-1 min-w-0">
                  {pipeline.segs.map(s => (
                    <p key={s.status} className="flex items-center gap-1.5" style={{ fontSize: 'var(--text-small)', color: 'var(--text)' }}>
