...
const Clients = () => {
  ...
  const { data: clients = [], isLoading } = useQuery({
    queryKey: ["clients"],
    queryFn: () => invokeFn<Client[]>("clients", { action: "list" }),
  });

  const clientMutation = useMutation({
    mutationFn: async () => {
      if (editingClient) {
        await invokeFn("clients", { action: "update", id: editingClient.id, name: clientName.trim() });
      } else {
        await invokeFn("clients", { action: "create", name: clientName.trim() });
      }
    },
    onSuccess: () => {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: ({ id, newStatus }: { id: string; newStatus: string }) =>
      invokeFn("clients", { action: "update", id, status: newStatus }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["clients"] });
      queryClient.invalidateQueries({ queryKey: ["auth-context"] });
  ...
  const deleteClientMutation = useMutation({
    mutationFn: (id: string) => invokeFn("clients", { action: "delete", id }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["clients"] });
      queryClient.invalidateQueries({ queryKey: ["auth-context"] });
