...
const Leads = () => {
  ...
  const { data: leads = [], isLoading } = useQuery({
    ...
    queryFn: () => invokeFn<Lead[]>("content", { action: "listLeads", clientId }),
    enabled: !!clientId,
  });
  ...
  const { data: sheetData = {}, isLoading: sheetLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      await Promise.all(sheetForms.map(async (form) => {
        ...
        try {
          const data = await invokeFn<{ headers: string[]; rows: Record<string, string>[] }>("content", {
            action: "syncGoogleSheet", clientId,
            sheetId: extractSheetId(cfg.sheet_url), colStart: cfg.col_start, colEnd: cfg.col_end,
          });
          if (data.headers) results[form.id] = data;
        } catch (err) {
    ...
    return (
      <div className="bg-card border border-border rounded-xl p-12 text-center">
        ...
        <p className="text-muted-foreground text-sm font-body">
          Ask your admin to assign you to a client account.
        </p>
      </div>
    );
  }
