...
function parseCSV(text: string): { url: string; outletName: string; uniqueVisits: number; totalVisits: number }[] {
  ...
  return lines.slice(1).map((line) => {
    ...
    return {
      url: cols[urlIdx] || "",
      outletName: outletIdx >= 0 ? cols[outletIdx] || "" : "",
      uniqueVisits: uniqueIdx >= 0 ? parseInt(cols[uniqueIdx]) || 0 : 0,
      totalVisits: totalIdx >= 0 ? parseInt(cols[totalIdx]) || 0 : 0,
    };
...
const PressReleases = () => {
  ...
  const { data: releases = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!clientId) return [];
      return invokeFn<PressRelease[]>("content", { action: "listPressReleases", clientId });
    },
    enabled: !!clientId,
  });
  ...
  const { data: allUrls = [] } = useQuery({
    queryKey: ["press-release-urls", releaseIds.join(",")],
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editingRelease) {
        await invokeFn("content", { action: "updatePressRelease", clientId, id: editingRelease.id, title: formTitle.trim(), content: formContent });
      } else {
        await invokeFn("content", { action: "createPressRelease", clientId, title: formTitle.trim(), content: formContent });
      }
    },
    onSuccess: () => {
  ...
  const submitMutation = useMutation({
    mutationFn: (id: string) => invokeFn("content", { action: "submitPressRelease", clientId, id }),
  ...
  const adminSaveMutation = useMutation({
    mutationFn: async () => {
      ...
      await invokeFn("content", {
        action: "updatePressRelease", clientId, id: adminRelease.id,
        title: adminTitle.trim(), content: adminContent, status: adminStatus,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => invokeFn("content", { action: "deletePressRelease", clientId, id }),
  ...
  const addUrlMutation = useMutation({
    mutationFn: ({ pressReleaseId, urls }: { pressReleaseId: string; urls: { url: string; outletName: string; uniqueVisits: number; totalVisits: number }[] }) =>
      invokeFn("content", { action: "addPressReleaseUrls", clientId, pressReleaseId, urls }),
    onSuccess: () => {
  ...
  function handleAddUrl() {
    ...
    addUrlMutation.mutate({
      ...
      urls: [{ url: newUrlValue.trim(), outletName: newOutletName.trim(), uniqueVisits: newUniqueVisits, totalVisits: newTotalVisits }],
    });
    setNewUrlValue("");
    setNewOutletName("");
    setNewUniqueVisits(0);
  ...
  if (!clientId) {
    return (
      <div className="bg-card border border-border rounded-xl p-12 text-center">
        ...
        <h3 className="font-display font-semibold text-foreground mb-1">{isAdmin ? "Select a client" : "No client account linked"}</h3>
        <p className="text-muted-foreground text-sm font-body">{isAdmin ? "Choose a client from the header." : "Ask your admin to assign you."}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground font-body text-sm">
          {isAdmin ? "Review and manage press releases." : "Manage press releases by quarter."} {totalPublished} published.
        </p>
      </div>
