...
import { testEdgeFunctionTool } from '../agent-tools/test_edge_function.js';
  ...
  for (const def of defs) {
    toolSet[def.name] = {
      ...
      execute: async (args: any) => {
        ...
        // ── Platform-URL guard ─────────────────────────────────────────────────
        // SMEsAgent infrastructure URLs (api/gen.SMEsAgent.dev, db/cloud/preview/
        // apps.SMEsAgent.app) must never be hardcoded into generated project code  
        ...
        // fallbacks like `import.meta.env.X || 'https://api.SMEsAgent.dev'`; this
        ...
        if ((def.name === 'write_file' || def.name === 'edit_file') && typeof args.path === 'string' && /\.(tsx?|jsx?)$/.test(args.path)) {
          const newContent = def.name === 'write_file'
            ? (typeof args.content === 'string' ? args.content : '')
            : (typeof args.diff === 'string' ? args.diff.replace(/<<<<<<< SEARCH[\s\S]*?=======/g, '') : '');
          const urlMatch = newContent.match(/https?:\/\/(?:[a-z0-9-]+\.)*SMEsAgent\.(?:dev|app|ai)\b/i);
          if (urlMatch) {
            return (
              `BLOCKED: "${args.path}" contains a hardcoded SMEsAgent platform URL (${urlMatch[0]}). ` +
          ...
          // not just a literal SMEsAgent domain   `window.location.origin`,
