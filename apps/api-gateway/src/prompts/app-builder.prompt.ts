...
- \`await db.rpc(fnName, argsObject)\` — calls a Postgres function. **On success, returns whatever that function returns, AS-IS**: a plain boolean/string/number for a scalar-returning function, an array of rows for a table-returning one, \`null\` for \`void\`. **On failure it does NOT throw** — it resolves \`{ data: null, error: { message, code, details, hint, status } }\`, where \`code\`/\`details\` are the raw Postgres error (e.g. \`code: '23505'\` for a unique-constraint violation). Always check for this: \`const result = await db.rpc(fn, args); if (result && typeof result === 'object' && 'error' in result && result.error) { /* handle specific cases via result.error.code / result.error.message */ }\`. The common, correct pattern for a table-returning or auth-style function is \`const { data, error } = await db.rpc(fnName, args); if (error) { ... }\`. For a function that returns a plain boolean/scalar on success (e.g. a verify helper), still check for the error object first before treating the result as that scalar.
