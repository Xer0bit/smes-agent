...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    const distortions: Record<string, any> = {
      ...
      mountainDistortion: {
        ...
        getDistortion: `
          ...
          vec3 getDistortion(float progress){
            float f=0.02;
            return vec3(cos(progress*PI*uFreq.x+uTime)*uAmp.x-cos(f*PI*uFreq.x+uTime)*uAmp.x, nsin(progress*PI*uFreq.y+uTime)*uAmp.y-nsin(f*PI*uFreq.y+uTime)*uAmp.y, nsin(progress*PI*uFreq.z+uTime)*uAmp.z-nsin(f*PI*uFreq.z+uTime)*uAmp.z);
          }
        `,
        getJS: (progress: number, time: number) => {
          const f = 0.02;
      ...
      xyDistortion: {
        ...
        getDistortion: `
          ...
          vec3 getDistortion(float progress){
            float f=0.02;
            return vec3(cos(progress*PI*uFreq.x+uTime)*uAmp.x-cos(f*PI*uFreq.x+uTime)*uAmp.x, sin(progress*PI*uFreq.y+PI/2.+uTime)*uAmp.y-sin(f*PI*uFreq.y+PI/2.+uTime)*uAmp.y, 0.);
          }
        `,
        getJS: (progress: number, time: number) => {
          const f = 0.02;
      ...
      LongRaceDistortion: {
        ...
        getDistortion: `
          ...
          vec3 getDistortion(float progress){
            float c=0.0125;
            return vec3(sin(progress*PI*uFreq.x+uTime)*uAmp.x-sin(c*PI*uFreq.x+uTime)*uAmp.x, sin(progress*PI*uFreq.y+uTime)*uAmp.y-sin(c*PI*uFreq.y+uTime)*uAmp.y, 0.);
          }
        `,
        getJS: (progress: number, time: number) => {
          const c = 0.0125;
          ...
          const d = new THREE.Vector3(
            Math.sin(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.sin(c * Math.PI * uFreq.x + time) * uAmp.x,
            Math.sin(progress * Math.PI * uFreq.y + time) * uAmp.y - Math.sin(c * Math.PI * uFreq.y + time) * uAmp.y,
            0,
          );
    ...
    const resizeRendererToDisplaySize = (renderer: THREE.WebGLRenderer, setSize: Function) => {
    ...
    const roadMarkings_fragment = `
      ...
      float laneWidth=1.0/uLanes; float brokenLineWidth=laneWidth*uBrokenLinesWidthPercentage; float laneEmptySpace=1.-uBrokenLinesLengthPercentage;
    ...
    class App {
      ...
      leftSticks: LightsSticks; fovTarget: number; speedUpTarget: number; speedUp: number;
      timeOffset: number; hasValidSize: boolean; renderPass: any; bloomPass: any;
      ...
      initPasses() {
        ...
        const smaaPass = new EffectPass(this.camera, new SMAAEffect({ preset: SMAAPreset.MEDIUM, searchImage: SMAAEffect.searchImageDataURL, areaImage: SMAAEffect.areaImageDataURL } as any));
    ...
    const options = {
      ...
      ...effectOptions,
      colors: { ...DEFAULT_EFFECT_OPTIONS.colors, ...(effectOptions as any).colors },
    };
    const distortionKey = (options as any).distortion as string;
    options.distortion = distortions[distortionKey] ?? distortions.turbulentDistortion;
  ...
  return <div id="hyperspeed-canvas" ref={containerRef} />;
};
