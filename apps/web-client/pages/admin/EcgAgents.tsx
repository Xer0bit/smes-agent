// eCG fleet visibility -- lists every SMEsAgent.dev dashboard connected to an
...
// agent-portal (mcp.SMEsAgent.ai) -- its admin routes are gated by an
...
import { Search, Bot, RefreshCw, Zap, ExternalLink } from 'lucide-react';
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Every SMEsAgent.dev dashboard connected to an eCG Agents org via MCP key, and which agent(s) it manages.
          </p>
        </div>
        ...
        <CardContent>
          {loading ? (
            <div className="flex items-center justify-center h-96">
              <BrandLoader variant="sonar" size={100} label="Loading agents" />
            </div>
          ) : error ? (
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/[0.06]">
                  <TableHead className="text-muted-foreground">Dashboard</TableHead>
                  <TableHead className="text-muted-foreground">Owner</TableHead>
                  <TableHead className="text-muted-foreground">eCG Org</TableHead>
                  <TableHead className="text-muted-foreground">Managed Agents</TableHead>
              ...
              <TableBody>
                {filtered.map((r) => (
                  <TableRow key={r.projectId} className="border-white/[0.04]">
                    ...
                    <TableCell>
                      <span className="text-sm text-muted-foreground">{r.ownerEmail ?? ' '}</span>
                      {r.orgName && <span className="block text-xs text-muted-foreground">{r.orgName}</span>}
                    </TableCell>
                    <TableCell>
                      <span className="text-sm text-foreground/80">{r.ecgOrgName ?? ' '}</span>
                    </TableCell>
                    <TableCell>
                      {r.agentCount === 0 ? (
                    ...
                    <TableCell>
                      <span className="text-xs text-muted-foreground">{r.modules.length > 0 ? r.modules.join(', ') : 'all'}</span>
                    </TableCell>
                    <TableCell>
                      <span className="text-sm text-muted-foreground">{r.updatedAt ? new Date(r.updatedAt).toLocaleDateString() : ' '}</span>
                    </TableCell>
