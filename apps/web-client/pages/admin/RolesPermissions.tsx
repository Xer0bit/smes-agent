...
const FEATURE_LIST: { key: keyof TierFeatures; label: string; description: string }[] = [
    ...
    { key: 'hosting',         label: 'Hosting',             description: 'One-click publish to SMEsAgent hosting' },
    { key: 'custom_domains',  label: 'Custom Domains',      description: 'Map custom domains to projects' },
    { key: 'remove_branding', label: 'Remove Branding',     description: 'Hide the "Made with SMEsAgent" footer' },
    ...
    { key: 'SMEsAgent_cloud',  label: 'SMEsAgent Cloud',      description: 'Managed cloud with custom domain' },
...
const PLAN_TIERS: { key: 'free' | 'pro' | 'agency'; label: string; color: string }[] = [
    { key: 'free',   label: 'Free',   color: 'text-foreground/80 border-border bg-gray-500/10' },
    { key: 'pro',    label: 'Pro',    color: 'text-muted-foreground border-border bg-muted' },
    { key: 'agency', label: 'Agency', color: 'text-muted-foreground border-border bg-muted' },
];
...
export default function AdminRolesPermissions() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-12">
            ...
            {/* ─── Save Status Bar ──────────────────────────────────────── */}
            <div className="rounded-none border p-3 flex items-center justify-between" style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(139,92,246,0.1)' }}>
                ...
                <Button variant="outline" size="sm" onClick={loadConfig} disabled={configLoading || saving} className="h-8 gap-2 border-white/10 bg-white/5 text-foreground hover:bg-white/10 text-xs">
            ...
            <div className="grid gap-6">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-muted-foreground">Toggle features on/off per subscription tier. Changes take effect immediately.</p>
                </div>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="border-white/[0.05] hover:bg-transparent">
                                        <TableHead className="w-[280px] text-muted-foreground text-xs font-semibold py-5 pl-6">Feature</TableHead>
                                        {PLAN_TIERS.map(tier => (
            ...
            <div className="grid gap-6">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-muted-foreground">Set numeric quotas per tier. For unlimited, enter 999999.</p>
                </div>
                    ...
                    <CardContent className="p-0">
                        {configLoading ? (
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="border-white/[0.05] hover:bg-transparent">
                                        <TableHead className="w-[280px] text-muted-foreground text-xs font-semibold py-5 pl-6">Limit</TableHead>
                                        {PLAN_TIERS.map(tier => (
                                            <TableHead key={tier.key} className="text-center py-5 text-xs font-semibold">
                                                <Badge className={cn('text-[11px] px-3 py-0.5 border font-semibold', tier.color)}>
                                                    {tier.label}
            ...
            <div className="grid gap-6 pt-4">
                <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="text-sm text-muted-foreground">User roles across all registered organizations.</p>
                    </div>
                </div>
                        ...
                        {rolesLoading ? (
                            ...
                            <div className="overflow-x-auto">
                                <Table>
                                    <TableHeader>
                                        <TableRow className="border-white/[0.05] hover:bg-transparent">
                                            <TableHead className="text-muted-foreground text-xs font-semibold py-4 pl-6">Member</TableHead>
                                            <TableHead className="text-muted-foreground text-xs font-semibold py-4">Organization</TableHead>
                                            <TableHead className="text-muted-foreground text-xs font-semibold py-4">Assigned Role</TableHead>
                                            <TableHead className="text-muted-foreground text-xs font-semibold py-4">Since</TableHead>
                                            <TableHead className="text-muted-foreground text-xs font-semibold py-4 text-right pr-6">Activity</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {roles.map((r) => {
                                            ...
                                            return (
                                                <TableRow key={r.id} className="border-white/[0.05] hover:bg-white/[0.02] transition-colors group">
                                                    ...
                                                    <TableCell className="text-xs text-muted-foreground font-medium">{new Date(r.created_at).toLocaleDateString()}</TableCell>
                                                    <TableCell className="text-right pr-6">
                                                        <Button variant="ghost" size="sm" className="h-8 text-muted-foreground hover:text-white group-hover:bg-white/5 opacity-0 group-hover:opacity-100 transition-all">
