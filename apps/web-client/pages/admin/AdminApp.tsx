    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-background">
                <BrandLoader variant="orbit" size={100} label="Authenticating" />
            </div>
        );
    }
                ...
                <div className="h-20 flex items-center px-5 relative overflow-hidden">
                    <div className="flex items-center gap-3 relative z-10 w-full">
                        <div className="h-10 w-10 flex-shrink-0  flex items-center justify-center group-hover:scale-110 transition-transform">
                            <img src={logo} alt="SMEs Agent" className="h-6 w-auto object-contain" />
                        </div>
                        <AnimatePresence mode="wait">
                            {!sidebarCollapsed && (
                                ...
                                >
                                    <span className="text-sm font-bold text-white tracking-widest uppercase leading-none">SMEsAgent</span>
                                    <span className="text-[10px] text-muted-foreground font-bold tracking-tighter uppercase mt-1">Platform Admin</span>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
                                ...
                                {group.items.map((item) => (
                                    <NavLink
                                        ...
                                        className={({ isActive }) => cn(
                                            "group relative flex items-center h-10 rounded-none text-sm transition-all duration-200 px-2.5",
                                            isActive
                                    ...
                                    >
                                        {({ isActive }) => (
                                            <>
                                                ...
                                                <div className={cn(
                                                    "h-7 w-7 rounded-none flex items-center justify-center shrink-0 transition-all duration-200",
                                                    isActive ? "bg-muted text-muted-foreground" : "text-muted-foreground group-hover:text-foreground/80"
                                                )}>
                                                ...
                                                {!sidebarCollapsed && isActive && (
                                                    <div className="absolute right-3 w-1 h-3 rounded-full bg-primary" />
                                                )}
                                            </>
                                        )}
                                    </NavLink>
                ...
                <div className="p-4 mt-auto border-t border-white/[0.04] bg-white/[0.01]">
                    ...
                    <div className="flex flex-col gap-2">
                        <Button
                            ...
                            className="h-10 justify-start px-3 text-muted-foreground hover:text-white hover:bg-white/5 rounded-none border border-transparent hover:border-white/5"
                        >
                        ...
                        <Button
                            ...
                            className="h-10 justify-start px-3 text-muted-foreground hover:text-rose-400 hover:bg-rose-500/10 rounded-none border border-transparent hover:border-rose-500/10"
                        >
                            <LogOut size={18} />
            ...
            <div className={cn(
                ...
                >
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="relative group hidden lg:block">
                            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 size-4 text-muted-foreground group-focus-within:text-muted-foreground transition-colors" />
                            <Input
                                placeholder="Search..."
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="h-11 w-80 pl-11 rounded-none bg-white/[0.03] border-white/10 text-xs text-white placeholder:text-muted-foreground/60 focus:border-primary/40 focus:bg-white/[0.05] transition-all shadow-inner"
                            />
                            <div className="absolute right-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded border border-white/10 text-[9px] text-muted-foreground/60 font-bold">
                                ⌘K
                            </div>
                        </div>

                        <div className="flex items-center gap-2 h-10 px-1 rounded-none bg-white/[0.03] border border-white/10">
                            ...
                            <Button
                                ...
                                className="h-8 px-3 text-[10px] font-bold uppercase tracking-wider text-muted-foreground hover:text-muted-foreground hover:bg-muted rounded-none flex items-center gap-2"
                ...
                <main className="flex-1 p-8 overflow-x-hidden relative">
                    ...
                    <footer className="mt-20 py-8 border-t border-white/[0.03] flex items-center justify-between text-[10px]">
                        <div className="flex items-center gap-4 text-muted-foreground/60 font-bold uppercase tracking-[0.2em]">
                            <span>© 2026 SMEsAgent Tech</span>
                            <div className="h-1 w-1 rounded-full bg-white/20" />
