...
function Stat({ icon: Icon, label, value, sub, tone }: {
  ...
  return (
    <div className="rounded-none border border-white/[0.06] bg-[hsl(var(--admin-surface))] p-5 flex items-center gap-4">
      <div className="h-10 w-10 rounded-none bg-white/[0.04] flex items-center justify-center shrink-0">
...
export default function AdminDelivery() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-12">
      <BrandLoader variant="cascade" size={100} label="Loading delivery health" />
    </div>
  );

  if (error) {
    return (
      <div className="rounded-none border border-red-500/25 bg-red-500/[0.06] p-5">
