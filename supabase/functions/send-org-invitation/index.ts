...
serve(async (req) => {
  ...
  try {
    ...
    const { org_name, inviter_name, email, token, role } = body;
    ...
    const displayOrg = (org_name as string | undefined)?.trim() || 'an organization';
    const displayInviter = (inviter_name as string | undefined)?.trim() || 'Someone';
    const displayRole = (role as string | undefined)?.trim() || 'member';

    const safeOrg = displayOrg.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const safeInviter = displayInviter.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const safeRole = displayRole.replace(/</g, '&lt;').replace(/>/g, '&gt;');
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f5;padding:40px 0;">
        ...
        <tr>
          <td style="background:#f9fafb;padding:16px 32px;border-top:1px solid #e4e4e7;">
            <p style="margin:0;font-size:11px;color:#a1a1aa;">
              Copyright ${new Date().getFullYear()} EcomGear ·
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: `${displayInviter} invited you to join ${displayOrg} on EcomGear`,
        html,
      }),
    });
