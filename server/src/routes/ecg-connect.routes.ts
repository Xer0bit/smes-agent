...
router.post('/', authMiddleware, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    // -- this row used to hold the raw file content directly, which silently
    // re-inflated a project's revision to tens of MB every time this endpoint
    // ran (confirmed on CardPro, 2026-07-22: 163 then 177 files, 19MB+ inline,
    // clobbering the same manifest-format fix applied to revisionService.ts).
    ...
    for (let i = 0; i < filesArray.length; i += UPLOAD_BATCH_SIZE) {
      ...
      await Promise.all(batch.map(async (f) => {
        ...
        const storagePath = `projects/${project.id}/${revisionId}/${f.path}`;
    ...
    await supabase.from('revisions').update({
      generated_files: { format: 'manifest-v1', files: manifestFiles, summary: `eCG dashboard for ${portalData.orgName}` },
