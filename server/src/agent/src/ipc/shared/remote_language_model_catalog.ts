...
import {
  ...
  SONNET_4_6,
  GEMINI_3_FLASH,
} from "./language_model_constants.js";
...
function canonicalizeModelApiName(apiName: string): string {
  switch (apiName) {
    ...
    case "claude-3-5-sonnet-20241022-latest":
...
function canonicalizeModelDisplayName(apiName: string, displayName: string): string {
  switch (canonicalizeModelApiName(apiName)) {
    case "gemini-2.5-pro":
      return "Gemini 2.5 Pro";
    case "gemini-2.5-flash":
      return "Gemini 2.5 Flash";
    case "claude-sonnet-4-6":
      return "Claude (EcomSmart)";
    default:
      return displayName;
  }
}
...
function convertRemoteCatalog(
  ...
  const modelsByProvider = Object.fromEntries(
    Object.entries(remoteCatalog.modelsByProvider).map(
      ([providerId, models]) => [
        providerId,
        models.map((model) => ({
          apiName: canonicalizeModelApiName(model.apiName),
          displayName: canonicalizeModelDisplayName(model.apiName, model.displayName),
          description: model.description,
          tag: model.tag,
          tagColor: model.tagColor,
          maxOutputTokens: model.maxOutputTokens,
  ...
  const mergedAliases = [
    ...remoteCatalog.aliases.map((alias) => ({
      ...alias,
      resolvedModel: {
        providerId: alias.resolvedModel.providerId,
        apiName: canonicalizeModelApiName(alias.resolvedModel.apiName),
      },
    })),
