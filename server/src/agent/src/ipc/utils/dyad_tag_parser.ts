...
import { SqlQuery } from "../../lib/schemas.js";
import { normalizeLegacyEcomgearTags } from "./cleanFullResponse.js";
...
export function getDyadWriteTags(fullResponse: string): {
  ...
  const normalizedResponse = normalizeLegacyEcomgearTags(fullResponse);
  ...
  while ((match = dyadWriteRegex.exec(normalizedResponse)) !== null) {
...
export function getDyadRenameTags(fullResponse: string): {
  ...
  while ((match = dyadRenameRegex.exec(normalizedResponse)) !== null) {
    tags.push({
...
export function getDyadCopyTags(fullResponse: string): {
  ...
  while ((match = dyadCopyRegex.exec(normalizedResponse)) !== null) {
    const attrs = match[1];
...
export function getDyadDeleteTags(fullResponse: string): string[] {
  ...
  let match;
  const paths: string[] = [];
  while ((match = dyadDeleteRegex.exec(normalizedResponse)) !== null) {
...
export function getDyadAddDependencyTags(fullResponse: string): string[] {
  ...
  while ((match = dyadAddDependencyRegex.exec(normalizedResponse)) !== null) {
...
export function getDyadChatSummaryTag(fullResponse: string): string | null {
  ...
  const match = dyadChatSummaryRegex.exec(normalizedResponse);
  if (match && match[1]) {
...
export function getDyadExecuteSqlTags(fullResponse: string): SqlQuery[] {
  ...
  while ((match = dyadExecuteSqlRegex.exec(normalizedResponse)) !== null) {
    const attributesString = match[1] || "";
    let content = unescapeXmlContent(match[2].trim());
    const descriptionMatch = descriptionRegex.exec(attributesString);
    const description = descriptionMatch?.[1]
...
export function getDyadCommandTags(fullResponse: string): string[] {
  ...
  while ((match = dyadCommandRegex.exec(normalizedResponse)) !== null) {
...
export function getDyadSearchReplaceTags(fullResponse: string): {
  ...
  while ((match = dyadSearchReplaceRegex.exec(normalizedResponse)) !== null) {
    const attributesString = match[1] || "";
    let content = unescapeXmlContent(match[2].trim());
