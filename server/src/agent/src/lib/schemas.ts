import { z } from "zod";
import { isOpenAIOrAnthropicSetup } from "./providerUtils.js";
 ...
 * - If defaultChatMode is "website-generator" but user doesn't have Pro:
 *   - Fall back to "build"
...
export function getEffectiveDefaultChatMode(
  ...
  if (settings.defaultChatMode) {
    if (settings.defaultChatMode === "website-generator") {
      return isPro ? "website-generator" : "build";
    }
