...
} from "./DyadMarkdownParser.js";
import { DyadAttachment, type AttachmentSize } from "./DyadAttachment.js";
import { useStreamChat } from "@/hooks/useStreamChat";
import { StreamingLoadingAnimation } from "./StreamingLoadingAnimation.js";
import {
  CheckCircle,
  XCircle,
  Clock,
...
import { unescapeXmlAttr } from "../../../shared/xmlEscape.js";
...
const ChatMessage = ({ message, isLastMessage }: ChatMessageProps) => {
  ...
  // 1) Never use fixed horizontal offsets (like ml-24) that can break on small screens.
  // 2) Constrain bubble width responsively to avoid cross-message overlap.
  // 3) Keep attachments aligned to the same side as their owner message.
  const isAssistant = message.role === "assistant";
  const rowClass = isAssistant ? "justify-start" : "justify-end";
  const columnClass = isAssistant
    ? "w-full max-w-3xl"
    : "w-full max-w-[min(92%,44rem)] sm:max-w-[min(86%,44rem)]";
  const bubbleClass = isAssistant
    ? "rounded-lg p-2"
    : "rounded-lg p-2 bg-(--sidebar-accent)";
  const attachmentRowClass = isAssistant
    ? "mt-2 flex flex-wrap gap-2 justify-start"
    : "mt-2 flex flex-wrap gap-2 justify-end";

  return (
    <div className={`w-full py-1.5 sm:py-2 flex ${rowClass}`}>
      <div className={`${columnClass} min-w-0 group`}>
        ...
        {(message.role === "assistant" || hasUserText) && (
          <div className={`${bubbleClass} min-w-0`}>
            {message.role === "assistant" &&
              ...
              <div
                className="prose dark:prose-invert prose-headings:mb-2 prose-p:my-1 prose-pre:my-0 prose-pre:overflow-x-auto max-w-none break-words text-[15px]"
            ...
            {(message.role === "assistant" &&
              ...
              <div
                className={`mt-2 flex items-center ${message.role === "assistant" &&
                    message.content &&
                    !isStreaming
                    ? "justify-between"
                    : ""
