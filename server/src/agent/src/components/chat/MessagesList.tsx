...
import { OpenRouterSetupBanner, SetupBanner } from "../SetupBanner.js";
...
import { PromoMessage } from "./PromoMessage.js";
...
function FooterComponent({ context }: { context?: FooterContext }) {
  ...
  const questionnaireState =
    chatId != null ? submittedChatIds.get(chatId) : undefined;
  ...
  return (
    <>
      ...
      {isStreaming &&
        ...
        messages.length > 0 && (
          <PromoMessage
            seed={messages.length * (appId ?? 1) * (chatId ?? 1)}
          />
        )}
...
export const MessagesList = forwardRef<HTMLDivElement, MessagesListProps>(
  function MessagesList(
    { chatId = null, messages, messagesEndRef, onAtBottomChange },
    ref,
  ) {
    ...
    const itemContent = useCallback(
      (index: number, message: Message) => {
        ...
        return (
          <div className="px-4 py-1" key={messageKey}>
            <MemoizedChatMessage
              message={message}
      ...
      return (
        ...
        >
          {messages.map((message, index) => {
            const isLastMessage = index === messages.length - 1;
            return (
              <div className="px-4 py-1" key={message.id}>
    ...
    return (
      <div
        className="absolute inset-0 p-4 pb-0 mb-2 overflow-hidden"
        ref={ref}
        data-testid="messages-list"
      >
        <Virtuoso
