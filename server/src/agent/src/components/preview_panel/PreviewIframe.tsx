...
import { AnnotatorOnlyForPro } from "./AnnotatorOnlyForPro.js";
import { useAttachments } from "@/hooks/useAttachments";
import { useUserBudgetInfo } from "@/hooks/useUserBudgetInfo";
import { Annotator } from "@/pro/ui/components/Annotator/Annotator";
import { VisualEditingToolbar } from "./VisualEditingToolbar.js";
...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
  ...
  const appUrlState = useAtomValue(appUrlAtom);
  const appUrl =
    selectedAppId !== null && appUrlState.appId === selectedAppId
      ? appUrlState.appUrl
      : null;
  const originalUrl =
    selectedAppId !== null && appUrlState.appId === selectedAppId
      ? appUrlState.originalUrl
      : null;
  ...
  const isSamePreviewBase = (currentUrl: string, nextAppUrl: string) => {
    try {
      const current = new URL(currentUrl);
      const next = new URL(nextAppUrl);
      ...
      const basePath = next.pathname.endsWith("/")
        ? next.pathname
        : `${next.pathname}/`;
  ...
  // But if the app URL itself changed to a different preview base/session, prefer
  // the fresh app URL so reloads don't stay pinned to a stale runtime.
  const iframeSrc = useMemo(() => {
    ...
    return isSamePreviewBase(currentUrl, appUrl) ? currentUrl : appUrl;
