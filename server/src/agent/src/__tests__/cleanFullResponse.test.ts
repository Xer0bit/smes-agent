...
AFTER TAG
  ...
  it("should normalize legacy EComGear tags to dyad tags", () => {
    const input = `<EComGear-write path="src/App.tsx">const App = () => null;</EComGear-write>`;
    const expected = `<dyad-write path="src/App.tsx">const App = () => null;</dyad-write>`;

    const result = cleanFullResponse(input);
    expect(result).toBe(expected);
  });

  it("should normalize lowercase ecomgear tags", () => {
    const input = `<ecomgear-add-dependency packages="react"></ecomgear-add-dependency>`;
    const expected = `<dyad-add-dependency packages="react"></dyad-add-dependency>`;
