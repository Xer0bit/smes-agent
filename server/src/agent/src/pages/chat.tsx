...
import { PreviewPanel } from "../components/preview_panel/PreviewPanel.js";
import { RightActionSidebar } from "../components/RightActionSidebar.js";
...
export default function ChatPage() {
  ...
  const chatAppIdByChatId = useMemo(
    () => new Map(chats.map((chat) => [chat.id, chat.appId])),
    [chats],
  );
  ...
  // Keep selected chat/app atoms in sync with route on reload/deep-link.
  // This guarantees chat filtering and tab state remain consistent after restart.
  useEffect(() => {
    if (!chatId) return;
    ...
    const knownAppId = chatAppIdByChatId.get(chatId);
    if (knownAppId !== undefined) {
      setSelectedAppId(knownAppId);
      return () => {
        cancelled = true;
      };
    }
    ...
    void ipc.chat
      ...
      .catch(() => {
        // Ignore not-found/navigation races; route-level handlers already manage navigation.
      });

    return () => {
      cancelled = true;
    };
  }, [chatId, chatAppIdByChatId, setSelectedAppId, setSelectedChatId]);
