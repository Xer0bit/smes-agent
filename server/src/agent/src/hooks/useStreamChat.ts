...
import { useLoadApp } from "./useLoadApp.js";
import { selectedAppIdAtom } from "@/atoms/appAtoms";
import { useVersions } from "./useVersions.js";
import { showExtraFilesToast } from "@/lib/toast";
import { useSearch } from "@tanstack/react-router";
import { useRunApp } from "./useRunApp.js";
import { useCountTokens } from "./useCountTokens.js";
import { useUserBudgetInfo } from "./useUserBudgetInfo.js";
import { usePostHog } from "posthog-js/react";
import { useCheckProblems } from "./useCheckProblems.js";
...
const INLINE_IMAGE_MARKDOWN_RE = /!\[([^\]]*)\]\(\s*(blob:[^)\s]+|data:[^)\s]+)\s*(?:"[^"]*")?\)/g;
...
async function materializeInlineImageAttachments(prompt: string): Promise<{
  prompt: string;
  attachments: FileAttachment[];
}> {
  const matches = Array.from(prompt.matchAll(INLINE_IMAGE_MARKDOWN_RE));
  if (matches.length === 0) {
  ...
  for (const match of matches) {
    ...
    if (!url || seenUrls.has(url)) {
      normalizedPrompt = normalizedPrompt.replace(fullMatch, "");
      continue;
    }
    ...
    try {
      ...
      const safeName = (altText || "inline-image")
        .trim()
        .replace(/[^a-zA-Z0-9._-]+/g, "-")
        .replace(/^-+|-+$/g, "") || "inline-image";
      const extension = imageExtensionFromMimeType(blob.type || "image/png");
      const file = new File([blob], `${safeName}.${extension}`, {
        type: blob.type || "image/png",
      });

      attachments.push({ file, type: "chat-context" });
      normalizedPrompt = normalizedPrompt.replace(fullMatch, "");
    } catch (error) {
      console.warn("[CHAT] Failed to materialize inline image attachment", error);
    }
  }

  return {
    prompt: normalizedPrompt.replace(/\s{2,}/g, " ").trim(),
    attachments,
  };
}
...
export function useStreamChat({
  ...
  const streamMessage = useCallback(
    async ({
      ...
      const normalizedPromptResult = await materializeInlineImageAttachments(prompt);
      const mergedAttachments = [
        ...(attachments ?? []),
        ...normalizedPromptResult.attachments,
      ];

      if ((!normalizedPromptResult.prompt.trim() && mergedAttachments.length === 0) || !chatId) {
        return;
      }
      ...
      try {
        ipc.chatStream.start(
          {
            chatId,
            prompt: normalizedPromptResult.prompt,
            redo,
