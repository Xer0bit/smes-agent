...
export const getDatabaseSchemaTool: ToolDefinition<z.infer<typeof schema>> = {
  ...
  execute: async (_args, ctx: AgentContext) => {
    ...
    return [
      ...
      functionLines,
      '',
      'IMPORTANT   every table above is deny-all by default (RLS enabled, zero policies) the instant it was created. ANON_KEY can read NOTHING until you explicitly run CREATE POLICY ... TO <schema>_anon for a table. If a table above has no such policy and your frontend fetches it directly, that fetch WILL return an empty array, not an error   this is the most common cause of "my page shows no data." Before treating a direct-fetch feature as done, either add the policy for genuinely public tables, or use write_edge_function for anything user-specific/private and for ALL writes.',
      '',
      'If you do use a direct client fetch (public read-only data only, table already has an explicit anon SELECT policy):',
