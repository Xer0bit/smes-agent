...
import { embedText, embedTexts, cosineSim } from './embedder.js';
import {
  upsertFileEmbedding,
  searchSimilarFiles,
  deleteFileEmbedding,
...
export async function retrieveRelevantFiles(
  ...
  try {
    ...
    console.warn('[kb/retrieval] vector/graph retrieval failed, using recency fallback:', err);
  }

  // 4. Recency fallback — fill remaining slots with most-recently-modified files
  if (results.filter(r => r.reason !== 'mentioned').length === 0) {
    const recency = [...allFiles]
      .sort((a, b) => b.content.length - a.content.length) // rough proxy for activity
      .slice(0, maxFiles);
    for (const f of recency) add(f.path, 0.1, 'recency');
  }
...
export function scoreFilesLocally(
  ...
  const scores = files.map(f => {
    const fileTokens = tokenize(`${f.path} ${f.content.slice(0, 800)}`);
    const fileSet = new Set(fileTokens);
    const overlap = promptTokens.filter(t => fileSet.has(t)).length;
    return {
      path: f.path,
      score: overlap / (promptTokens.length + 1),
