...
- **Partially addressed (2026-08-05 stability review):** the tenant-schema tracking above (`supabase_migrations.schema_migrations`) is for the *platform's own* Postgres schema only. Per-tenant database schemas (one per provisioned project, `server/src/services/database.service.ts`) still have no real migration/versioning system — no per-schema `up`/`down` semantics, no rollback path. What now exists: a `tenant_schema_migrations` audit table (`supabase/migrations/20260805070000_tenant_schema_migrations.sql`) that `runQuery()` writes an append-only row to (schema, project, SQL text, statement count, timestamp) whenever the agent's `query_database` tool runs DDL — pure visibility ("what ran, when, for which project"), not a rollback mechanism. Note `runQuery()` already executes each call transactionally (real `BEGIN`/`COMMIT`, not auto-committed statement-by-statement), so the sharper "half-migrated schema" risk was smaller than this gap implied — the audit table exists to make investigation possible, not to fix a corruption risk that was already contained. `provision()`/`deprovision()` (a different code path, not agent-DDL) were also made transactional in the same pass, closing a related-but-separate orphaned-schema/role risk on those two operations specifically.
