    ...
    <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          {lastChecked && <p className="text-xs text-gray-500 mt-0.5">Last checked {lastChecked}</p>}
        </div>
        <button
          onClick={runChecks}
          className="text-xs text-blue-400 hover:text-blue-300 border border-gray-600 rounded px-2.5 py-1 transition-colors"
        >
          Refresh
        </button>
      </div>
  ...
  return (
    <div className="rounded border border-gray-700 overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-gray-700/40 transition-colors text-left"
      >
      ...
      {expanded && (
        <div className="border-t border-gray-700 bg-gray-900/60 px-4 py-3 space-y-1">
          <p className="text-xs text-gray-500 font-mono break-all">{r.url}</p>
          {r.error ? (
            <p className="text-xs text-red-400 mt-1">{r.error}</p>
          ) : (
            <pre className="text-xs text-gray-300 whitespace-pre-wrap break-all mt-1 font-mono max-h-40 overflow-y-auto">
              {r.body || '(empty body)'}
            </pre>
          )}
        </div>
...
function TroubleshootPanel() {
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-5 space-y-5">
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={running}
          className="flex items-center gap-2 text-sm text-white bg-purple-600 hover:bg-purple-500 disabled:opacity-50 rounded px-3 py-1.5 transition-colors"
        >
          {running ? (
            <>
      ...
      <div>
        <p className="text-xs text-gray-400 font-medium mb-2">Manual Endpoint Probe</p>
        <div className="flex gap-2">
          <select
            value={probeMethod}
            onChange={e => setProbeMethod(e.target.value as 'GET' | 'POST')}
            className="bg-gray-900 border border-gray-600 rounded text-xs text-gray-300 px-2 py-1.5"
          >
            <option>GET</option>
            <option>POST</option>
          </select>
          <input
            ...
            placeholder="https://gen.ecomgear.dev/health"
            className="flex-1 bg-gray-900 border border-gray-600 rounded text-xs text-gray-300 px-3 py-1.5 placeholder:text-gray-600 focus:outline-none focus:border-purple-500"
          />
          <button
            onClick={runManualProbe}
            disabled={probeRunning || !probeUrl.trim()}
            className="text-xs text-white bg-gray-700 hover:bg-gray-600 disabled:opacity-50 rounded px-3 py-1.5 transition-colors"
          >
            {probeRunning ? '…' : 'Test'}
          </button>
        </div>
        {probeMethod === 'POST' && (
          <textarea
            ...
            rows={2}
            className="mt-2 w-full bg-gray-900 border border-gray-600 rounded text-xs text-gray-300 font-mono px-3 py-2 placeholder:text-gray-600 focus:outline-none focus:border-purple-500 resize-none"
          />
        )}
